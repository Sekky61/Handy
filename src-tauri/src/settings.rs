...
const DEFAULT_IMPROVE_TRANSCRIPTIONS_PROMPT: &str = r#"<transcript>
${output}
</transcript>
...
6. Preserve programming and technical terms, commands, library and product names, abbreviations, and code identifiers, even when they appear in a Czech sentence. Keep the original technical word and adapt only its grammatical ending when needed; do not replace it with a Czech-sounding word.
...
In Czech sentences, programming words are often used as loanwords and conjugated in Czech. For example:
Before: Řekl jsem rybejsuju, model to rozpoznal jako rybejsuju
After: Řekl jsem rebasuju, model to rozpoznal jako rebasuju

Example:
Before: Přečti si išuju a udělej podle něj entend testy
After: Přečti si issue a udělej podle něj E2E test
...
fn ensure_post_process_defaults(settings: &mut AppSettings) -> bool {
    ...
    // Update the built-in prompt for existing installations, but never
    // overwrite a prompt the user has customized.
    if let Some(prompt) = settings
        .post_process_prompts
        .iter_mut()
        .find(|prompt| prompt.id == "default_improve_transcriptions")
    {
        if prompt.prompt == LEGACY_IMPROVE_TRANSCRIPTIONS_PROMPT {
...
mod tests {
    ...
    #[test]
    fn default_post_process_prompt_preserves_czech_programming_terms() {
        let prompt = &get_default_settings().post_process_prompts[0].prompt;

        assert!(prompt.contains("programming and technical terms"));
        assert!(prompt.contains("Řekl jsem rybejsuju"));
        assert!(prompt.contains("Řekl jsem rebasuju"));
    }

    #[test]
    fn legacy_default_post_process_prompt_is_migrated() {
        let mut settings = get_default_settings();
        settings.post_process_prompts[0].prompt = LEGACY_IMPROVE_TRANSCRIPTIONS_PROMPT.to_string();
    ...
    #[test]
    fn customized_default_post_process_prompt_is_not_overwritten() {
        let mut settings = get_default_settings();
        settings.post_process_prompts[0]
            .prompt
            .push_str(" Custom rule.");
